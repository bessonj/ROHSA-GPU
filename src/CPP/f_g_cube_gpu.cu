...
void prepare_for_convolution_sort(double* d_IMAGE_amp, double* d_IMAGE_amp_ext, int image_x, int image_y, dim3 BlocksParGrille_init, dim3 ThreadsParBlock_init, dim3 BlocksParGrille, dim3 ThreadsParBlock)
{
/*
...
//    print_device_array<double><<<BlocksParGrille,ThreadsParBlock>>>(d_IMAGE_amp, image_x, image_y);
    init_extended_array_sort<double><<<BlocksParGrille,ThreadsParBlock>>>(d_IMAGE_amp, d_IMAGE_amp_ext, image_x, image_y);
//    print_device_array_sort<double><<<BlocksParGrille,ThreadsParBlock>>>(d_IMAGE_amp, image_x, image_y);
    extension_mirror_gpu_sort<double><<<BlocksParGrille_init,ThreadsParBlock_init>>>(d_IMAGE_amp, d_IMAGE_amp_ext, image_x, image_y);
//    print_device_array<double><<<BlocksParGrille,ThreadsParBlock>>>(d_IMAGE_amp_ext, image_x+4, image_y+4);
}

void conv_twice_and_copy_sort(double* d_IMAGE_amp_ext, double* d_conv_amp, double* d_conv_conv_amp, int image_x, int image_y, dim3 BlocksParGrille_init, dim3 ThreadsParBlock_init, dim3 BlocksParGrille, dim3 ThreadsParBlock)
{
...
//    d_conv_amp <- d_RESULTAT_first_conv
    copy_gpu_sort<double><<<BlocksParGrille, ThreadsParBlock>>>(d_conv_amp, d_RESULTAT_first_conv, image_x, image_y);
//    init_extended_array_sort<double><<<BlocksParGrille_init,ThreadsParBlock_init>>>(d_conv_amp, d_RESULTAT_first_conv, image_x, image_y);
//    extension_mirror_gpu_sort<double><<<BlocksParGrille,ThreadsParBlock>>>(d_conv_amp, d_RESULTAT_first_conv, image_x, image_y);
    prepare_for_convolution_sort(d_conv_amp, d_RESULTAT_first_conv, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
...
//
//
void conv2D_GPU_all_sort(parameters& M, double* d_g, int n_beta, double lambda_var_sig, double* b_params_dev, double* deriv_dev, double* beta_modif_dev, double* array_f_dev, long int image_x, long int image_y, int n_gauss, float temps_transfert, float temps_mirroirs)
{
for(int k = 0; k<M.n_gauss; k++){
    ...
    int threads = BLOCK_SIZE_S;
    int blocks_x  = ceil(double(image_x+4) / double(BLOCK_SIZE_S));
    int blocks_y  = ceil(double(image_y+4) / double(BLOCK_SIZE_S));
    int blocks_x_init = ceil(double(max(image_x+4, image_y)) / double(BLOCK_SIZE_S));
    int blocks_y_init = 2;//ceil(double(2) / double(BLOCK_SIZE_S));
...
*/
    prepare_for_convolution_sort(d_IMAGE_amp, d_IMAGE_amp_ext, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
    prepare_for_convolution_sort(d_IMAGE_mu, d_IMAGE_mu_ext, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
    prepare_for_convolution_sort(d_IMAGE_sig, d_IMAGE_sig_ext, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
...
//    printf("\n\t Running GPU 2D-Convolution algorithm for %lu iterations\n", nb_cycles);
    ...
    conv_twice_and_copy_sort(d_IMAGE_amp_ext, d_conv_amp, d_conv_conv_amp, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
    conv_twice_and_copy_sort(d_IMAGE_mu_ext, d_conv_mu, d_conv_conv_mu, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
    conv_twice_and_copy_sort(d_IMAGE_sig_ext, d_conv_sig, d_conv_conv_sig, image_x, image_y, BlocksParGrille_init, ThreadsParBlock_init, BlocksParGrille, ThreadsParBlock);
...
*/
    ...
    dim3 Dg_ud, Db_ud;
    Db_ud.x = BLOCK_SIZE_X; //x
    Db_ud.y = BLOCK_SIZE_Y; //y
    Dg_ud.x = ceil(double(image_x)/double(BLOCK_SIZE_X));
    Dg_ud.y = ceil(double(image_y)/double(BLOCK_SIZE_Y));
//    printf("Dg_ud = %d, %d, %d ; Db_ud = %d, %d, %d\n",Dg_ud.x,Dg_ud.y,Dg_ud.z,Db_ud.x,Db_ud.y,Db_ud.z);
    ...
    kernel_conv_g_sort<<<1,1>>>(n_beta, d_g, d_image_sigma_reduc_f, M.lambda_var_sig, n_gauss, b_params_dev, k, image_x*image_y);
...
*/
    ...
    reduction_loop_parallel(map_norm_dev, d_array_f, indice_x*indice_y);
