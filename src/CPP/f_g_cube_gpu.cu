...
#include "kernels_for_paralell.cu"
...
template <typename T> void prepare_for_convolution_sort(T* d_IMAGE_amp, T* d_IMAGE_amp_ext, int image_x, int image_y, dim3 BlocksParGrille_init, dim3 ThreadsParBlock_init, dim3 BlocksParGrille_frame, dim3 ThreadsParBlock_frame)
{
    ...
    if(print){
        ...
        printf("display d_IMAGE_amp :\n");
            checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_amp, 600);
            checkCudaErrors(cudaDeviceSynchronize());
        std::cin.ignore();
    }
    ...
    if(print){
        ...
        printf("display d_IMAGE_amp_ext :\n");
            checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_amp_ext, 600);
            checkCudaErrors(cudaDeviceSynchronize());
        std::cin.ignore();
    }
...
//    printf("Dg_ud = %d, %d, %d ; Db_ud = %d, %d, %d\n",Dg_ud.x,Dg_ud.y,Dg_ud.z,Db_ud.x,Db_ud.y,Db_ud.z);
    ...
    if(indice_x>=256 && print){
        ...
        display_dev_complete_sort<T><<<1,1>>>(map_conv_dev,15);
        checkCudaErrors(cudaDeviceSynchronize());
...
/*
    if(indice_x>128){
        ...
        checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(map_dev,4);//indice_x*indice_y);
//        display_dev_complete_sort<T><<<1,1>>>(map_norm_dev,4);//indice_x*indice_y);
        checkCudaErrors(cudaDeviceSynchronize());
...
*/
    ...
    if(indice_x>=256 && print){
        ...
        checkCudaErrors(cudaDeviceSynchronize());
        display_dev_sort<T><<<1,1>>>(array_f_dev_bis);
        display_dev_complete_sort<T><<<1,1>>>(map_dev,15);
        checkCudaErrors(cudaDeviceSynchronize());
        printf("Fin f convolution\n");
        checkCudaErrors(cudaDeviceSynchronize());
        std::cin.ignore();
...
//    display_dev_complete_sort<T><<<1,1>>>(beta_modif_dev, 30);
...
/*
    display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_amp, image_x*image_y);
    display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_mu, image_x*image_y);
    display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_sig, image_x*image_y);
*/
    checkCudaErrors(cudaDeviceSynchronize());
...
//    printf("\n IMAGE EXT\n");
/*
    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_amp_ext, image_x+4,image_y+4);
    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_mu_ext, image_x+4,image_y+4);
    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_sig_ext, image_x+4,image_y+4);

    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_mu, image_x,image_y);
    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_sig, image_x,image_y);

    display_dev_complete_2D_sort<T><<<1,1>>>(d_conv_conv_amp, image_x, image_y);
    display_dev_complete_2D_sort<T><<<1,1>>>(d_conv_conv_mu, image_x,image_y);
    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_sig, image_x,image_y);

    display_dev_complete_sort<T><<<1,1>>>(b_params_dev, M.n_gauss);
    display_dev_complete_sort<T><<<1,1>>>(d_g, n_beta);
    display_dev_complete_2D_sort<T><<<1,1>>>(d_IMAGE_sig, image_x, image_y);
    ...
    display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_amp_ext, 660);
        checkCudaErrors(cudaDeviceSynchronize());
    printf("print amp map\n");
    if(print){
        std::cin.ignore();
        ...
        printf("print amp map\n");
            checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_amp, 300);
            checkCudaErrors(cudaDeviceSynchronize());
        printf("print mu map\n");
            checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_mu, 300);
            checkCudaErrors(cudaDeviceSynchronize());
        printf("print sig map\n");
            checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(d_IMAGE_sig, 300);
            checkCudaErrors(cudaDeviceSynchronize());
    }
*/
    if(print){
        printf("print conv amp map\n");
            checkCudaErrors(cudaDeviceSynchronize());
        display_dev_complete_sort<T><<<1,1>>>(d_conv_amp, 300);
            checkCudaErrors(cudaDeviceSynchronize());
    }
/*
    printf("print conv mu map\n");
        checkCudaErrors(cudaDeviceSynchronize());
    display_dev_complete_sort<T><<<1,1>>>(d_conv_mu, 300);
        checkCudaErrors(cudaDeviceSynchronize());
    printf("print conv sig map\n");
        checkCudaErrors(cudaDeviceSynchronize());
    display_dev_complete_sort<T><<<1,1>>>(d_conv_sig, 300);
        checkCudaErrors(cudaDeviceSynchronize());
    printf("display at index d_conv_amp[idx] :\n");
        checkCudaErrors(cudaDeviceSynchronize());
    display_dev_at_index<T><<<1,1>>>(d_conv_amp, image_x*image_y-2);
    display_dev_at_index<T><<<1,1>>>(d_conv_amp, image_x*image_y-1);
    display_dev_at_index<T><<<1,1>>>(d_conv_amp, image_x*image_y);
*/
        checkCudaErrors(cudaDeviceSynchronize());
...
//    display_dev_complete_sort<T><<<1,1>>>(d_conv_conv_amp, 30);
    kernel_update_deriv_conv_conv_sort<T><<<Dg_ud,Db_ud>>>(deriv_dev, M.lambda_amp, M.lambda_mu, M.lambda_sig, M.lambda_var_sig, d_conv_conv_amp, d_conv_conv_mu, d_conv_conv_sig, d_IMAGE_sig, b_params_dev, int(image_y), int(image_x),k);
//    display_dev_complete_sort<T><<<1,1>>>(deriv_dev, 30);
//	std::cin.ignore();
    ...
    kernel_conv_g_reduction_sort<T><<<1,1>>>(n_beta, d_g, d_image_sigma_reduc, M.lambda_var_sig, n_gauss, b_params_dev, k, image_x, image_y);
//    kernel_conv_g_sort<T><<<Dg_ud,Db_ud>>>(n_beta, d_g, d_IMAGE_sig, M.lambda_var_sig, n_gauss, b_params_dev, k, image_x, image_y);
...
//gradient_kernel_2_beta_working_sort<T><<<Dg,Db>>>(deriv_dev, taille_deriv_dev, beta_modif_dev, taille_beta_modif_dev, residual_dev, taille_residual_dev, std_map_dev, taille_std_map_dev, n_gauss);

//    display_dev_complete_sort<<<1,1>>>(beta_modif_dev,taille_deriv[2]*taille_deriv[1]*taille_deriv[0]);

//display_dev_sort<T><<<1,1>>>(beta_modif_dev);
//    display_dev_complete_sort<T><<<1,1>>>(beta_modif_dev,10);

    gradient_kernel_2_beta_with_INDEXING_sort<T><<<Dg,Db>>>(deriv_dev, taille_deriv_dev, beta_modif_dev, taille_beta_modif_dev, residual_dev, taille_residual_dev, std_map_dev, taille_std_map_dev, n_gauss);
//    gradient_kernel_2_beta_with_INDEXING_sort_edit<T><<<Dg,Db>>>(deriv_dev, taille_deriv_dev, beta_modif_dev, taille_beta_modif_dev, residual_dev, taille_residual_dev, std_map_dev, taille_std_map_dev, n_gauss);
...
template <typename T> void reduction_loop_parallel(T* array_in, T* d_array_f, int size_array){
    ...
    reduce_last_in_one_thread_sort<T><<<1,1>>>(array_in, d_array_f, size_array);
/*
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    if (N==1){
        ...
        sum_reduction_sort<T><<< GRID_SIZE_REDUCTION, BLOCK_SIZE_REDUCTION >>>(array_in, array_out_kernel, size_array);

        checkCudaErrors(cudaDeviceSynchronize());

        cpy_first_num_dev_sort<T><<<1,1>>>( array_out_kernel, d_array_f);
        ...
        int copy_dev_blocks = ceil(T(size_array_out_kernel)/T(BLOCK_SIZE_REDUCTION));
        copy_dev_sort<T><<< copy_dev_blocks , BLOCK_SIZE_REDUCTION >>>(array_out_kernel, array_in_copied_2, size_array_out_kernel);
        ...
        sum_reduction_sort<T><<< GRID_SIZE_REDUCTION, BLOCK_SIZE_REDUCTION >>>(array_in_copied_2, array_out_kernel_2, size_array_out_kernel);

        checkCudaErrors(cudaDeviceSynchronize());

        if(N>2){
        reduce_last_in_one_thread_sort<T><<<1,1>>>(array_out_kernel_2, d_array_f, size_array_out_kernel_2);
        }
        else{
        cpy_first_num_dev_sort<T><<<1,1>>>( array_out_kernel_2, d_array_f);
        }
...
//    kernel_residual_sort<T><<<Dg,Db>>>(beta_dev, cube_dev, residual_dev,indice_x, indice_y, indice_v, n_gauss);
    ...
    kernel_norm_map_boucle_v_sort<T><<<Dg_L2, Db_L2>>>(map_norm_dev, residual_dev, std_map_dev, indice_x, indice_y, indice_v);
