...
{ 
    ...
    int tr2=t_r[2]; // v --> i
	int tr1=t_r[1]; // y --> index_z
	int tr0=t_r[0]; // x --> index_y

	int ts0=t_std[0]; // y --> index_z
	int ts1=t_std[1]; // x --> index_y

	int td0 = t_d[0]; // ng --> index_x
	int td1 = t_d[1]; // y --> index_z
	int td2 = t_d[2]; // x --> index_y

	int tp0 = t_p[0]; // y --> index_z
	int tp1 = t_p[1]; // x --> index_y
	int tp2 = t_p[2]; // ng --> index_x

//						reality			dev world 
        //params     --> (y,x,ng)    --> (z,y,x)
		//residual   --> (x,y,z)     --> (y,z,i)
		//deriv      --> (ng,y,x)    --> (x,z,y)
		//std_map    --> (y,x)       --> (z,y)
////  printf("ERROR 2 - ");
	if(index_x<n_gauss && index_y<td2 && index_z<td1 && std_map[index_z*ts1+index_y]>0.)
	{
		...
		double par2_pow = 1/(2*pow(par2,2.));
		double parstd = 1/pow(std_map[index_z*ts1+index_y],2);
...
////			printf("i = %d , index_z = %d , index_y = %d , index_x = %d\n",i,index_z,index_y,index_x);
			...
			buffer_0 += exp( -pow( par1 ,2.)*par2_pow ) * residual[index_y*tr1*tr2+index_z*tr2+i]*parstd;
			buffer_1 += par0*par1*par2_pow*2 * exp(-pow( par1,2.)*par2_pow ) * residual[index_y*tr1*tr2+index_z*tr2+i]*parstd;
			buffer_2 += par0*pow( par1, 2.)/(pow(par2,3.))*exp(-pow(par1 ,2.)*par2_pow ) * residual[index_y*tr1*tr2+index_z*tr2+i]*parstd;
		}



		deriv[(3*index_x+0)*td1*td2+index_z*td2+index_y]=buffer_0;
		deriv[(3*index_x+1)*td1*td2+index_z*td2+index_y]=buffer_1;
		deriv[(3*index_x+2)*td1*td2+index_z*td2+index_y]=buffer_2;
		__syncthreads();
	}
