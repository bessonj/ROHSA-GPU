...
template <typename T> void f_g_cube_cuda_L_clean_templatized_less_transfers(parameters<T> &M, T& f, T* g, int n, T* beta, int indice_v, int indice_y, int indice_x, std::vector<std::vector<T>> &std_map, T* cube_flattened, double* temps, double temps_transfert_d, double temps_mirroirs, float* temps_kernel);
template <typename T> void f_g_cube_cuda_L_clean_templatized_no_transfers(parameters<T> &M, T& f, T* g_dev, int n, T* beta_dev, int indice_v, int indice_y, int indice_x, T* std_map_dev, T* cube_flattened_dev, double* temps, double temps_transfert_d, double temps_mirroirs, float* temps_kernel);
//template <typename T> void f_g_cube_cuda_L_clean_templatized_no_transfers(parameters<T> &M, T& f, T* g, int n, T* beta, int indice_v, int indice_y, int indice_x, std::vector<std::vector<T>> &std_map, T* cube_flattened, double* temps, double temps_transfert_d, double temps_mirroirs, float* temps_kernel);
